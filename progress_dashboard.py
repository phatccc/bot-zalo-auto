...
class ProgressTracker:
    ...
    def delete_batch(self, batch_id: str) -> bool:
        with self._lock:
            ...
            return len(self._batches) < before
...
<script>
const route=location.pathname,content=document.querySelector('#content'),nav=document.querySelector('#nav'),summary=document.querySelector('#summary');let open=new Set(),itemsClosed=new Set(),last={};
...
function batchCard(b){let total=Math.max(1,+b.total||1),done=(+b.success||0)+(+b.failed||0),percent=Math.min(100,Math.round(done*100/total)),opened=open.has(b.id),iopen=!itemsClosed.has(b.id),issues=b.issues||[];return `<article class="batch ${b.done?'is-done':''} ${issue(b)?'has-issue':''} ${opened?'open':''} ${iopen?'items-open':''}"><div class="batch-top"><div class="mark">${b.done?'✓':'↗'}</div><div class="copy"><div class="name">${esc(b.main_acc)}</div><div class="meta">Người gửi: ${esc(b.sender_name)} · ${esc(b.started_at)}</div></div><span class="state">${b.done?'Hoàn tất':'Đang chạy'}</span><button class="btn" data-open="${esc(b.id)}">${opened?'Thu gọn':'Chi tiết'}</button><button class="btn" data-items="${esc(b.id)}">${iopen?'▲ Ẩn ảnh':'▼ Xem ảnh ('+b.total+')'}</button>${b.done?`<button class="btn btn-del" data-del="${esc(b.id)}">Xóa</button>`:''}</div><div class="batch-body"><div class="progress"><span>${esc(b.stage)}</span><span>${done}/${total} ảnh</span></div><div class="bar"><div class="fill" style="width:${percent}%"></div></div><div class="facts"><span class="fact">${b.success||0} thành công</span>${b.failed?`<span class="fact bad">${b.failed} lỗi ảnh</span>`:''} ${issues.length?`<span class="fact bad">${issues.length} cảnh báo</span>`:''}</div><div class="batch-issues">${issues.map(issueCard).join('')}</div><div class="items">${(b.items||[]).map(x=>`<div class="item ${itemType(x.stage)}"><b>Ảnh ${String(x.position).padStart(2,'0')}</b><span title="${esc(x.detail||x.stage)}">${esc(x.stage)}</span></div>`).join('')}</div></div></article>`}
function dashboard(d){let all=d.batches||[],seen=new Map(),visible=new Set();for(let x of [...all].reverse()){if(issue(x)){visible.add(x);continue}let key=[x.sender_name,x.main_acc,x.total].join('\u0001'),at=Number(String(x.id||'').split('-')[0])||0,first=seen.get(key);if(!first||at-first.at>300000){seen.set(key,{at});visible.add(x)}}let b=[...all.filter(x=>visible.has(x)&&!x.done),...all.filter(x=>visible.has(x)&&x.done)];content.innerHTML=`<div class="head"><h2>Tiến độ batch</h2></div><div class="list">${b.length?b.map(batchCard).join(''):'<div class="empty"><strong>Không có batch phù hợp</strong><p>Bot đang chờ ảnh và bảng giá mới.</p></div>'}</div>`;content.querySelectorAll('[data-open]').forEach(x=>x.onclick=()=>{open.has(x.dataset.open)?open.delete(x.dataset.open):open.add(x.dataset.open);dashboard(last)});content.querySelectorAll('[data-items]').forEach(x=>x.onclick=()=>{itemsClosed.has(x.dataset.items)?itemsClosed.delete(x.dataset.items):itemsClosed.add(x.dataset.items);dashboard(last)});content.querySelectorAll('[data-del]').forEach(x=>x.onclick=async()=>{await fetch('/api/progress/'+x.dataset.del,{method:'DELETE'});load();})}
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_DELETE(self):  # noqa: N802
            ...
            if path.startswith("/api/progress/"):
                batch_id = path[len("/api/progress/"):]
